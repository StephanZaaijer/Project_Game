...
{
	prev_state = gameData->json.Get_Musicstate();
	if (!prev_state) {
	...
	if (horizontal) {
		...
		prev_sliderblock_location = { slider_location.x + length - (gameData->json.Get_Musicvolume()) * ratio, slider_location.y };
	}
	else {
		slider_min = slider_location.y;
		slider_max = slider_location.y + length;
		ratio = ((slider_max - slider_min) / 100);
		prev_sliderblock_location = { slider_location.x, slider_location.y + length - (gameData->json.Get_Musicvolume()) * ratio };
...
void Musicslider::update() {
	...
	if (prev_sliderblock_location != slider_block.getPosition()){
		if (horizontal) {
			...
			level.setOrigin({ level.getGlobalBounds().width / 2, level.getGlobalBounds().height / 2 });
			gameData->json.Set_Musicvolume(current_level);
		}
		else {
			gameData->json.Set_Musicstate(true);
